...
from .forms import UserForm,DonorSignupForm,LoginForm,MyPasswordChangeForm,DonateNowForm,VolunteerSignupForm,DonationAreaForm
from django.contrib.auth.models import User
from .models import Donor,Donation,Volunteer,DonationArea,Gallery
...
def volunteerallocated_donation(request):
    ...
    if not request.user.is_authenticated:
            return redirect('/login-admin')
    donation = Donation.objects.filter(status='Volunteer Allocted')
...
def donationnotrec_admin(request):
    if not request.user.is_authenticated:
            return redirect('/login-admin')
    donation = Donation.objects.filter(status='Donation NotRecieved')
...
class profile_donor(View):
    ...
    def post(self, request):
            ...
            try:
                ...
                messages.success(request,"profile Updated Failed"+e)
            return render(request, "profile-donor.html",locals())    
...
def donationnotrec_volunteer(request):
        ...
        user = request.user
        volunteer = Volunteer.objects.get(user=user)
        donation=Donation.objects.filter(volunteer=volunteer,status="Donation NotRecieved")
